"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var http_1 = require("@angular/http");
var router_1 = require("@angular/router");
var AddAlbumComponent = (function () {
    function AddAlbumComponent(http, r) {
        var _this = this;
        this.http = http;
        this.r = r;
        this.model = new Album();
        this.router = r;
        http.get('/api/artists1').subscribe(function (result) {
            _this.artists = result.json();
        });
        http.get('/api/genres1').subscribe(function (result) {
            _this.genres = result.json();
        });
    }
    AddAlbumComponent.prototype.onSubmit = function (form) {
        var _this = this;
        var headers = new http_1.Headers();
        headers.append('Content-Type', 'application/json');
        this.http.post('/api/albums1/', JSON.stringify(this.model), { headers: headers }).subscribe(function (res) { return _this.postResponse = res.json(); });
        this.router.navigate(['./albums']);
    };
    return AddAlbumComponent;
}());
AddAlbumComponent = __decorate([
    core_1.Component({
        selector: 'addalbum',
        templateUrl: './addalbum.component.html'
    }),
    __metadata("design:paramtypes", [http_1.Http, router_1.Router])
], AddAlbumComponent);
exports.AddAlbumComponent = AddAlbumComponent;
var Album = (function () {
    function Album() {
    }
    return Album;
}());
//# sourceMappingURL=addalbum.component.js.map